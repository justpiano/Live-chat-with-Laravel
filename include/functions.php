...
function jak_chatgpt_chat($usermsg, $gptmsgs, $widgettopic, $widgetid, $lang, $depid) {
    ...
    // ChatGPT History
    if (!empty($gptmsgs)) foreach( $gptmsgs as $msg ) {
        $conversation[] = [
            "role" => "system",
            "content" => $msg["message"]
        ];
    ...
    if ($response === false) {
        ...
        // Now we store the message in the gtp table to make of use later.
        global $jakdb;
        $jakdb->insert("bot_chatgpt", ["widgetids" => $widgetid,
            "depid" => $depid,
            "lang" => $lang,
            "question" => $usermsg,
            "answer" => $botReply,
